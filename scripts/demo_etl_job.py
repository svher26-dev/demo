...
# ── Read source tables ──────────────────────────────────────────────────────
print("=== Reading source tables from S3 Tables Iceberg ===")
customers = spark.table(f"{CATALOG}.{NAMESPACE}.demo_customers")
orders    = spark.table(f"{CATALOG}.{NAMESPACE}.demo_orders")
products  = spark.table(f"{CATALOG}.{NAMESPACE}.demo_products")
...
job.commit()
print("=== ETL complete — 3 curated tables written to Aurora ===")
